...
def test_openapi_artifact_is_current_and_scoped_to_nc011() -> None:
    ...
    expansion = document["paths"]["/api/v1/me/network/expand"]["post"]
    assert any(
        parameter["name"] == "X-Network-Compass-Persona" for parameter in expansion["parameters"]
    )
    for error_status in ("400", "401", "422", "500"):
        response_schema = expansion["responses"][error_status]["content"]["application/json"][
            "schema"
        ]
        assert response_schema == {"$ref": "#/components/schemas/ErrorResponseSchema"}
